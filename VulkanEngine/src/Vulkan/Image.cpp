...
namespace VE
{
    ...
    Image::Image(const Device &deivce, const VkExtent2D extent, const VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage)
        : m_Device(deivce), m_Extent(extent), m_Format(format), m_ImageLayout(VK_IMAGE_LAYOUT_UNDEFINED)
    {
        ...
        CheckVk(vkCreateImage(m_Device.Handle(), &imageInfo, nullptr, &m_Image), "create image!");
    }
    ...
    DeviceMemory Image::AllocateMemory(const VkMemoryPropertyFlags properties) const
    {
        ...
        CheckVk(vkBindImageMemory(m_Device.Handle(), m_Image, memory.Handle(), 0), "bind image memory!");
